...
const Navbar = () => {
  ...
  const handleLogout = async () => {
    setDropdownOpen(false);
    try { await authService.logout(); } catch { /* cookie cleared */ }
    logout();
    navigate('/');
  };
  ...
  const isAdmin = user?.dutyId === 'admin';
          ...
          <div className="user-dropdown-wrap" ref={dropdownRef}>
            ...
            {dropdownOpen && (
              <div className="user-dropdown">
                {!user ? (
                  <>
                    <Link to="/login" className="dropdown-item dropdown-item-main" onClick={() => setDropdownOpen(false)}>
                      เข้าสู่ระบบ / สมัครสมาชิก <ChevronRight size={14} />
                    </Link>
                    <div className="dropdown-divider" />
                    <Link to="/login"    className="dropdown-item" onClick={() => setDropdownOpen(false)}>เข้าสู่ระบบ</Link>
                    <Link to="/register" className="dropdown-item" onClick={() => setDropdownOpen(false)}>สมัครสมาชิก</Link>
                  </>
                ) : (
                  <>
                    <div className="dropdown-user-info">
                      <span className="dropdown-user-name">{user.memName}</span>
                      <span className="dropdown-user-role">{isAdmin ? 'Admin' : 'Member'}</span>
                    </div>
                    <div className="dropdown-divider" />
